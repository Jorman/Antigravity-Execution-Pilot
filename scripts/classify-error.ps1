...
function Classify-CommandError {
    param(
        [string]$Command = "",
        [int]$ExitCode = 1,
        [string]$Stderr = "",
    ...
    elseif ($combinedErr -match "(?i)(Authentication failed|Invalid credentials|Unauthorized|Permission denied \(publickey\)|Login failed|Access denied for user)") {
        $category = "authentication_required"
        $cause = "Authentication or credentials required"
        $remedy = "Verify authentication tokens, API keys, SSH keys, or login credentials"
    }
...
if ($MyInvocation.InvocationName -ne '.') {
    $res = Classify-CommandError -Command $Command -ExitCode $ExitCode -Stderr $Stderr -Stdout $Stdout -WorkingDir $WorkingDir
